  ...
  },
  {
    ...
    contributions: {
      es: [
        ...
        'Desarrollé el motor de resiliencia y sincronización offline en el frontend (React 19 + IndexedDB) con cola de peticiones locales, reconciliación de conflictos y modularización basada en Feature-based architecture con lazy loading.',
        'Diseñé e implementé el flujo de partial pick fulfillment con creación de tareas residuales en la misma orden (from_location_id nullable, soporte de available_quantity negativo) e idempotency keys en Task y StockMovement.',
        'Construí el módulo de notificaciones de alto valor WMS (5 alertas RBAC, consolidación de expiraciones en lote) y el sistema de reporte de incidencias con resiliencia offline (IndexedDB).',
      ],
      en: [
        ...
        'Developed the frontend offline resilience and synchronization engine (React 19 + IndexedDB) featuring local request queues, conflict reconciliation and Feature-based modularization with lazy loading.',
        'Designed and implemented the partial pick fulfillment workflow with residual task creation within the same order (nullable from_location_id, negative available_quantity support) and idempotency keys on Task and StockMovement.',
        'Built the high-value WMS notifications module (5 RBAC alert types, batch expiration consolidation) and the incident reporting system with offline resilience (IndexedDB).',
      ],
    },
